...
public class ZenInsight extends Activity {
    ...
    private static int IMAGE_VIEW = -1;
    private static int TEXT_VIEW = -2;

    //defining states
    private static int[] sCamSteps = { IMAGE_VIEW, FRONT_CAMERA, BACK_CAMERA, FRONT_CAMERA, BACK_CAMERA, FRONT_CAMERA, IMAGE_VIEW, TEXT_VIEW};
    private static int[] sStringSteps = { R.string.step0text, R.string.step1text, R.string.step2text, R.string.step3text, R.string.step4text, R.string.step5text, R.string.step6text, R.string.step7text };
...
// Disable text speech option - it sounds strange
...
*/
    ...
    private Camera attachCamera(FrameLayout holder, int camera) {
    	if (camera == BACK_CAMERA || camera == FRONT_CAMERA) {
	    	try {
    	...
    	} else if (camera == IMAGE_VIEW) {
    		ImageView view = new ImageView(this);
    		view.setImageResource(R.drawable.enzo);
    		view.setTag(mViewfinder);
    		holder.addView(view);
    	} else if (camera == TEXT_VIEW) {
            TextView view = new TextView(this);
            view.setText(R.string.bigtext);
            view.setTag(mViewfinder);
            holder.addView(view);
        }
    	return null;
